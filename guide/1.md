...
The parsing is done in `parser.rb`. It is your job to implement the `parse` function here. A lot of the gritty work of counting parentheses and such has been done for you, but you must stitch everything together.

- Have a look at the provided functions in `ruby-lisp/parser.rb` before you start. These should prove useful. 
- The following command runs the tests.

    ```bash
    ruby spec/test_1_parser_spec.rb
    ```
- Run the tests and hack away until all the tests are passing. Each test has a description, and you should probably read it if you get stuck.
